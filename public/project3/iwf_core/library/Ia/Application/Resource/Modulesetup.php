<?php
class Ia_Application_Resource_ModuleSetup
    extends Zend_Application_Resource_ResourceAbstract
{
    ...
    protected function _getModuleSetup()
    {
        ...
        foreach (array_keys($modules) as $module) {
            ...
            if ($module!=='default' && file_exists($configPath)) {
                        ...
                        if($filename=='module.ini'){
                            ...
                            if (strtolower($cfgtype) == 'module') {
                                if (array_key_exists($module, $appOptions)) {
                                    if (is_array($appOptions['modules'][$module])) {
                                        $appOptions['modules'][$module] =
                                            array_merge($appOptions['modules'][$module], $options);
                                    } else {
                                        $appOptions['modules'][$module] = $options;
                                    }
                                } else {
                                    $appOptions['modules'][$module] = $options;
                                }
                            } else {
                                $appOptions['modules'][$module]['resources'][$cfgtype] = $options;
                            }                            
