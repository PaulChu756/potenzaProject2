 ...
 * 
 */
class Ia_PDF_Merger extends Ia_PDF
{
	private $_files;	//['form.pdf']  ["1,2,4, 5-19"]
	private $_fpdi;
    private $_iaPdf = null;
    
    public function setIaPdf($iaPdf){
        $this->_iaPdf = $iaPdf;
    }
    
    public function getIaPdf()
    {
        if($this->_iaPdf===null){
            $this->_iaPdf = new Ia_Pdf;
        }
        return $this->_iaPdf;
    }
	...
	{
        // Automatically validate and repair if necessary
        if(!$this->getIaPdf()->validate($filepath)){
            $filepath = $this->getIaPdf()->repair($filepath);
        }    
	...
	public function merge($outputmode = 'browser', $outputpath = 'newfile.pdf')
	{
		...
		else
		{
			...
			{
				throw new exception("Error outputting PDF '$outputpath' to '$outputmode'");
				return false;
			}
		}
